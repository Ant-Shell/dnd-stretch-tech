...
const MonsterSection: FC<Props> = (props) => {
  ...
  const [currentMonster, setCurrentMonster] = useState<MonsterStructure>({
    ...
    xp: 5900,
    proficiencies: [
      { value: 6, proficiency: { name: "Saving Throw: CON" } },
      { value: 8, proficiency: { name: "Saving Throw: INT" } },
      { value: 6, proficiency: { name: "Saving Throw: WIS" } },
      { value: 12, proficiency: { name: "Skill: History" } },
      { value: 10, proficiency: { name: "Skill: History" } },
    ],
    special_abilities: [
  ...
  const monsterHandler = (
    ...
    const theMonster = props.monsters.find(
      (monster) => monster.name === (event.target as HTMLParagraphElement).id
    );
  ...
  const monsterSearchHandler = (monsterName: string) => {
    const theMonster = props.monsters.find(
      (monster) => monster.name.toUpperCase() === monsterName.toUpperCase()
    );
  ...
  return (
    <div className="monster-section">
      ...
      <MonsterDetails
        ...
        monsterSearchHandler={monsterSearchHandler}
