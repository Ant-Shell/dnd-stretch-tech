...
const MonsterDetails: FC<Props> = ({
  currentMonster,
  monsterSearchHandler,
  monsterNotFound,
}) => {
  ...
  const specialAbilities = currentMonster.special_abilities.map((spec) => {
    return (
      <div className="special" key={spec.desc}>
        <h3 className="special-ability-name">{spec.name}</h3>{" "}
        <p className="special-description">{spec.desc}</p>
      </div>
    );
  });

  const monsterNotFoundMesage = monsterNotFound === true && (
    <span className="search-error-message">Monster not found.</span>
  );
        ...
        <div>
          <header className="head">
            ...
            <div className="search-bar-container">
              ...
              ></input>
              <button
                className="monster-search-button"
                onClick={() => monsterSearchHandler(searchInput)}
              >
                SEARCH
              </button>
        ...
        <div className="stat-ability-wrapper">
          <div className="monster-stats">
            ...
            <p className="monster-stat">
              Challenge Rating: {currentMonster.challenge_rating}
            </p>
            ...
            <p className="monster-stat">
              Alignment: {currentMonster.alignment}
            </p>
            <p className="monster-stat">
